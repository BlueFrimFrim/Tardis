...
SoftwareSerial tardisSS = SoftwareSerial(TARDIS_TX, TARDIS_RX);
SoftwareSerial *tardisSerial = &tardisSS;
...
void print_diagnostic_menu(void)
{
  Serial.println(F("-------------------------------------"));
  Serial.println(F("[?] Print this menu"));
...
void loop(void)
{
	Serial.println(F("TARDIS:Master"));
	Serial.print(F("$ "));
}
...
void loop() {
  ...
  switch (command) {
    ...
    case 'T': {
        ...
        Serial.println();
        // play for 5 second (5000 ms)
        if (! fona.playToolkitTone(kittone, 5000)) {
          Serial.println(F("Failed"));
        } else {
          Serial.println(F("OK!"));
        }
